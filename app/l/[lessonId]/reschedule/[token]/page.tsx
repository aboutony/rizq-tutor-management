...
export default async function RescheduleLessonPage({ params }: { params: { lessonId: string, token: string } }) {
  const { lessonId, token } = params;
  const messages = getMessages();
  const lessonDetails = await getLessonDetailsByToken(token, 'reschedule');
  ...
  return (
    <main className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-lg mx-auto">
        <RescheduleForm lessonDetails={lessonDetails} messages={messages} token={token} />
      </div>
    </main>
  );
}
