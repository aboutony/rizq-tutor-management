...
export default async function RateLessonPage({ params }: { params: { lessonId: string, token: string } }) {
  const { lessonId, token } = params;
  const messages = getMessages();
  const lessonDetails = await getLessonDetailsByToken(token, 'rate');
  ...
  return (
    <main className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-lg mx-auto">
        <RatingForm lessonDetails={lessonDetails} messages={messages} token={token} />
      </div>
    </main>
  );
}
